...
const PremiumKey = "HSBDBD7373HB3B49292BEBB3"
...
async function SBVHtear() {
	...
	VH.on('credentials-updated', () => {
		...
		fs.writeFileSync('./data1.json', JSON.stringify(authInfo, null, '\t'))
	})
	fs.existsSync('./data1.json') && VH.loadAuthInfo('./data1.json')
	await VH.connect()
	...
	VH.on('chat-update', async(chat) => {
		...
		if (m.key.fromMe) {
			if (txt == "hi") {
				...
				VHsendMessage(to,`Speed: ${processTime(t, moment())} _Second_`)
			} else if (txt == "leave") {
				VHsendMessage(VH, to, "See you...")
				VH.leaveGroup(to)
			} else if (txt == "me") {
				VHsendContact(VH, to, "Salzbots", VH.user.jid)
			} else if (txt == "creator") {
				var sal = "6282279014545"
				VHsendContact(VH, to, "This Is Creator Sb", sal)
			} else if (txt == "tagall") {
